...
public class CreateApartmentCommandValidation : AbstractValidator<CreateApartmentCommand>
{
    public CreateApartmentCommandValidation()
    {
        ...
        RuleFor(r => r.TypeOfBooking).NotEmpty().WithMessage("Field must not be empty")
            .MinimumLength(5).WithMessage("Type Of Booking must be at least 5 symbols")
            .MaximumLength(25).WithMessage("Type Of Booking must be less than 25 symbols");
        RuleFor(r => r.StreetName).NotEmpty().WithMessage("Field must not be empty")
            .MinimumLength(5).WithMessage("StreetName must be at least 5 symbols")
            .MaximumLength(25).WithMessage("StreetName must be less than 25 symbols");
